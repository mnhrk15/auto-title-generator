...
- 短いタイトル枠はプロンプトでの指示（＝目標）であり保証ではありません。AIが長いタイトルばかり返すと、選択した語が**どのタイトルにも含まれない**ことがあります。その場合はレスポンスの `unapplied_season_keywords` に該当語が入り、生成結果の上部に「選択した『春カラー』はタイトルの文字数制約により今回の結果には付与されませんでした」という注釈バナーが表示されます（サーバログにも warning が残ります）
...
`unapplied_season_keywords` には、選択した季節・カラーのうち **どのタイトルにも
含まれなかった語**（`"春カラー"` などの付加語文言）が入ります。後処理で付加できなくても
生成タイトルに元から含まれていれば未付与とは数えません。該当語がなければ空配列です。
フロントエンドはこれを読んで生成結果上部に注釈バナーを表示します。
...
- **季節・カラー後処理**: `apply_season_keywords()`（`app/seasons.py`）が生成後のタイトルへ選択キーワードを均等配分で付加
