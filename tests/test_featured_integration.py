...
class TestFeaturedKeywordsAPI:
    ...
    def test_get_featured_keywords_with_error(self, client):
        ...
        with patch_featured_repository() as mock_manager:
            # モックの設定（エラー状態）
            mock_manager.is_available.return_value = False
            mock_manager.get_last_error.return_value = FeaturedKeywordsLoadError(
                "ファイル読み込みエラー"
            )
            mock_manager.get_health_status.return_value = {
                'is_available': False,
...
class TestTemplateGenerationAPI:
    ...
    def mock_scraper_results(self):
        """モックスクレイピング結果"""
        return ["くびれヘアスタイル1", "くびれヘアスタイル2", "くびれヘアスタイル3"]
    ...
    def test_generate_with_featured_keyword(
        self, client, mock_scraper_results, mock_template_results
    ):
        """テンプレート生成API - 特集キーワードでの生成テスト"""
        with (
            patch_featured_repository() as mock_featured_manager,
            ...
            mock_generator.generate_templates_async = AsyncMock(
                return_value=(mock_featured_templates, [])
            )
            ...
            assert (
                data['featured_keyword_info']['condition']
                == 'スタイル名に『くびれヘア』を含めること。'
            )
    ...
    def test_generate_with_normal_keyword(
        self, client, mock_scraper_results, mock_template_results
    ):
        ...
        with (
            ...
            mock_generator.generate_templates_async = AsyncMock(
                return_value=(mock_normal_templates, [])
            )
            mock_generator_class.return_value = mock_generator
    ...
    def test_generate_with_mixed_keywords(
        self, client, mock_scraper_results, mock_template_results
    ):
        ...
        with (
            ...
            mock_generator.generate_templates_async = AsyncMock(
                return_value=(mock_mixed_templates, [])
            )
            mock_generator_class.return_value = mock_generator
...
class TestEndToEndIntegration:
    ...
    def test_featured_keywords_to_template_generation_flow(self, client):
        ...
        with (
            ...
            keyword_found = any(
                'くびれヘア' in template.get('title', '') for template in generate_data['templates']
            )
    ...
    def test_error_recovery_and_logging(self, client):
        ...
        with (
            ...
            # HotPepperScraperのモック設定
            mock_scraper = AsyncMock()
            mock_scraper.scrape_titles_async.return_value = ["フォールバックヘアスタイル1"]
            mock_scraper.__aenter__.return_value = mock_scraper
            mock_scraper.__aexit__.return_value = None
            mock_scraper_class.return_value = mock_scraper
