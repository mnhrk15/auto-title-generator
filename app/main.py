...
def get_featured_keywords():
    ...
    try:
        ...
        if not repository.is_available():
            ...
            if last_error:
                ...
                if isinstance(last_error, featured_keywords.FeaturedKeywordsLoadError):
                    ...
                    message = (
                        '特集キーワード機能で問題が発生しています。管理者にお問い合わせください。'
                    )
            else:
        ...
        current_app.logger.info(
            f'性別フィルタリング後: {len(filtered_keywords)}件 (対象: {gender})'
        )
        ...
        current_app.logger.error(
            f'特集キーワード取得中に予期しないエラーが発生しました: {str(e)}', exc_info=True
        )
...
async def generate():
    ...
    try:
        ...
        data = (
            request.get_json()
        )  # ここで BadRequest が発生する可能性があり、app_errorhandler(400) で処理される
        if (
            data is None
        ):  # request.get_json() はパース失敗時にNoneを返すことがある (force=Falseの場合など)。通常はBadRequest。
        ...
        if not isinstance(seasons, list):
            raise ValidationError(
                '季節・カラーの指定形式が正しくありません。配列で指定してください。'
            )

        current_app.logger.info(
            f'テンプレート生成リクエスト - キーワード: "{keyword}", 性別: "{gender}", 季節・カラー選択: {seasons}, モデル: "{model}"'
        )
        ...
        if gender not in GENDERS:
            raise ValidationError(
                '無効な性別が指定されました。ladies または mens を指定してください。'
            )
