...
### 最重要: 文字数の厳守
各要素は上限ギリギリまで文字数を使い切ってください。短すぎるのも、超過するのもNGです。
- title: ちょうど{config.CHAR_LIMITS['title']}文字に近づける（**27〜30文字**。26文字以下は短すぎ）
- menu: ちょうど{config.CHAR_LIMITS['menu']}文字に近づける（**45〜50文字**。44文字以下は短すぎ）
- comment: ちょうど{config.CHAR_LIMITS['comment']}文字に近づける（**90〜120文字**。120文字を絶対に超えない）
...
**ターゲット層:**
「20代30代」「30代40代」など年代を入れると検索に効果的です。全タイトルに入れる必要はありませんが、{config.MAX_TEMPLATES}個中**半分以上**を目安に取り入れてください。
...
### 【コメント】（ちょうど{config.CHAR_LIMITS['comment']}文字に近づける、90〜120文字）
